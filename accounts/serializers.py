...
class TeacherSearchSerializer(serializers.ModelSerializer):
    ...
    last_name = serializers.CharField(
        source="user_profile.user.last_name", read_only=True
    )
...
class GigTeacherSerializer(serializers.ModelSerializer):
    ...
    native_language = serializers.CharField(
        source="user_profile.native_language", read_only=True
    )
...
class GigSerializer(serializers.ModelSerializer):
    ...
    def get_pricing_metadata(self, obj):
        try:
            ...
            return {
                "platform_fee_percentage": getattr(
                    settings, "PLATFORM_FEE_PERCENTAGE", 0.05
                ),
                "minimum_platform_fee_dollars": getattr(
                    settings, "MINIMUM_PLATFORM_FEE_CENTS", 100
                )
                / 100,
                "currency": "USD",
            }
