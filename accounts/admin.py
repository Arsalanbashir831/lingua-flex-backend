...
class TeacherProfileAdmin(admin.ModelAdmin):
    ...
    def qualification_short(self, obj):
        return (
            obj.qualification[:50] + "..."
            if len(obj.qualification) > 50
            else obj.qualification
        )
...
class GigAdmin(admin.ModelAdmin):
    ...
    search_fields = (
        "service_title",
        "short_description",
        "teacher__user_profile__user__email",
    )
    ...
    fieldsets = (
        ...
        ("Pricing & Duration", {"fields": ("price_per_session", "session_duration")}),
        ("Tags & Status", {"fields": ("tags", "status")}),
    )
    ...
    def draft_gigs(self, request, queryset):
        ...
        self.message_user(
            request, f"Successfully moved {queryset.count()} gigs to draft."
        )
