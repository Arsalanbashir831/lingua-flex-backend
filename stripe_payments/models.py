...
class Payment(models.Model):
    ...
    session_booking = models.OneToOneField(
        "bookings.SessionBooking", on_delete=models.CASCADE, related_name="payment"
    )
    student = models.ForeignKey(
        "core.User", on_delete=models.CASCADE, related_name="payments_made"
    )
    ...
    session_duration_hours = models.DecimalField(
        max_digits=5, decimal_places=2, help_text="Session duration in hours"
    )
    platform_fee_cents = models.IntegerField(
        default=0, help_text="Platform fee in cents"
    )
...
class SavedPaymentMethod(models.Model):
    ...
    student = models.ForeignKey(
        "core.User", on_delete=models.CASCADE, related_name="saved_payment_methods"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_default:
            SavedPaymentMethod.objects.filter(
                student=self.student, is_default=True
...
class RefundRequest(models.Model):
    ...
    requested_amount_cents = models.IntegerField(
        help_text="Requested refund amount in cents"
    )
...
class StripeCustomer(models.Model):
    ...
    user = models.OneToOneField(
        "core.User", on_delete=models.CASCADE, related_name="stripe_customer"
    )
