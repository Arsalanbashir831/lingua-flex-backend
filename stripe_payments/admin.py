...
class PaymentAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (
            ...
            {
                "fields": (
                    "payment_status_badge_display",
                    "amount_display_large",
                    "platform_fee_display_large",
                    "calculate_fees",
                )
            },
        ),
        ...
        ("Relationships", {"fields": ("session_booking", "student", "teacher", "gig")}),
        (
            "Stripe Integration",
            {
                "fields": (
        ...
        ("Refunds", {"fields": ("refund_history",), "classes": ("collapse",)}),
    ...
    def booking_link(self, obj):
        ...
        if obj.session_booking:
            url = reverse(
                "admin:bookings_sessionbooking_change", args=[obj.session_booking.id]
            )
            return format_html(
                '<a href="{}" target="_blank">Booking #{}</a>',
                url,
                obj.session_booking.id,
            )
        return "-"
    ...
    def platform_fee_display_large(self, obj):
        ...
        return format_html(
            '<div style="font-size: 18px; color: #ff9800;">${:.2f} ({:.1f}%)</div>',
            fee,
            percentage,
        )
    ...
    def calculate_fees(self, obj):
        ...
        stripe_fee = amount * Decimal("0.029") + Decimal(
            "0.30"
        )  # Approximate Stripe fee
        ...
        for payment in queryset:
            writer.writerow(
                [
                    ...
                    payment.session_booking.scheduled_datetime.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                    if payment.session_booking
                    and payment.session_booking.scheduled_datetime
                    else "",
...
class RefundRequestAdmin(admin.ModelAdmin):
    ...
    list_filter = [
        ...
        "created_at",
        "reviewed_at",
        "payment__session_booking__status",
    ]
    search_fields = [
        "student__email",
        ...
        (
            "Refund Overview",
            {
                "fields": (
                    ...
                    "urgency_badge_display",
                    "requested_amount_display_large",
                    "refund_calculation",
                )
            },
        ),
    ...
    def status_badge_display(self, obj):
        """Large status badge for detail view"""
        return format_html(
            '<div style="font-size: 18px;">{}</div>', self.status_badge(obj)
        )
    ...
    def refund_calculation(self, obj):
        ...
        platform_fee_portion = (Decimal(obj.payment.platform_fee_cents) / 100) * (
            requested / original
        )
    ...
    def export_refunds_csv(self, request, queryset):
        ...
        self.message_user(
            request, f"Exported {queryset.count()} refund requests to CSV."
        )
        return response
...
class SavedPaymentMethodAdmin(admin.ModelAdmin):
    ...
    search_fields = ["student__email", "stripe_payment_method_id", "card_last_four"]
    readonly_fields = [
